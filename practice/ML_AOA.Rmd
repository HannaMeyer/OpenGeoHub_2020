...
predplot <- spplot(deratify(prediction_mr), col.regions=cols, main = list(label="Prediction (left), prediction only for the AOA (right) and RGB composite (bottom)",cex=0.8))
predplotaoa <- spplot(deratify(prediction_mr), col.regions=cols)+
spplot(AOA$AOA,col.regions=c("grey","transparent"))
